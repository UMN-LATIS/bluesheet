    ...
    <div>
        <div class="row">
          <div class="col-md-12">
            ...
            <ul class="groupInfo">
                <li v-if="group.parent_organization && $can('view groups')">Folder: <strong><router-link :to="{ name: 'groupList', params: { parent: group.parent_organization.id } }">{{ group.parent_organization.group_title }}</router-link></strong></li>
                ...
                <li v-if="$can('schedule departments')"><router-link :to="{'name':'schedulingReport', params: { groupId: group.id }}">View Scheduling Report</router-link></li>
            </ul>
        </div>
    </div>
